...
interface DayCellProps {
  presence?: Presence;
  onAddPresence: (userId: number, dateStr: string) => void;
  userId: number;
  dateStr: string;
  isEditable: boolean;
}

export const DayCell = memo(({ presence, onAddPresence, userId, dateStr, isEditable }: DayCellProps) => {
  ...
  return (
    <div
      onClick={isEditable ? () => onAddPresence(userId, dateStr) : undefined}
      className={`w-full h-full flex items-center justify-center transition-colors p-2 group/cell ${isEditable ? 'cursor-pointer lg:hover:bg-base-200' : 'cursor-default'}`}
      title={presence ? `${t('daycell.status')}: ${getDynamicCategoryName(presence.categories, i18n.language, t)}` : undefined}
    >
      {presence ? (
