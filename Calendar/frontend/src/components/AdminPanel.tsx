            ...
            <tbody>
              {users.map((user) => (
                <tr key={user.id_user} className="hover:bg-base-200/30 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="avatar">
                        <div className="w-10 h-10 rounded-xl bg-base-200 border border-base-300 shadow-sm overflow-hidden">
                          <img 
                            src={user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.alias || user.full_name )}&background=random&color=fff`} 
                            alt={user.alias || user.full_name} 
                  ...
                  <td>
                    <span className={`badge font-bold text-[10px] uppercase ${user.role === 'admin' || user.role === 'ADMIN' ? 'badge-primary' : 'badge-ghost'}`}>
                      {user.role}
                    </span>
                  </td>
                  <td className="text-right">
                    <button onClick={() => handleOpenUserModal(user)} className="btn btn-circle btn-ghost btn-sm hover:bg-info/20 hover:text-info">
                      <EditIcon fontSize="small" />
                    </button>
                    <button onClick={() => handleDeleteUser(user.id_user, user.full_name)} className="btn btn-circle btn-ghost btn-sm hover:bg-error/20 hover:text-error">
          ...
          {categories.map((cat) => (
            <div key={cat.id_category} className="bg-base-100 border border-base-300 rounded-3xl p-5 flex items-center justify-between hover:shadow-xl transition-all group">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => handleDeleteCategory(cat.id_category, cat.name)} className="btn btn-circle btn-ghost btn-sm hover:bg-error/20 hover:text-error">
                  <DeleteIcon fontSize="small" />
                </button>
              </div>
            </div>
            ...
            <form onSubmit={handleSaveCategory} className="space-y-6">
              <div className="form-control">
                ...
                <input 
                  ...
                  required
                />
                <p className="text-[10px] mt-2 opacity-40 px-1">Usa el nombre original de la base de datos para que la traducción automática funcione.</p>
              </div>
                  ...
                  {availableIcons.map(emoji => (
                    <button
                      ...
                      className={`btn btn-circle btn-lg text-2xl transition-all ${
                        categoryFormData.icon === emoji ? 'btn-primary scale-110 shadow-md ring-4 ring-primary/20' : 'btn-ghost hover:bg-base-300'
                      }`}
                    >
        ...
        <div className="modal modal-open backdrop-blur-md">
          <div className="modal-box rounded-[32px] border border-base-300 shadow-2xl p-8 max-w-md">
            <h3 className="font-bold text-2xl text-base-content mb-6 tracking-tight">
              {userFormData.id_user ? t('admin.edit_user') : t('admin.new_user')}
            </h3>
            <form onSubmit={handleUserSubmit} className="space-y-4">
              <div className="form-control">
                ...
                <input type="text" required placeholder="Mario Rossi" className="input input-bordered w-full rounded-2xl bg-base-200/50" value={userFormData.full_name} onChange={e => setUserFormData({ ...userFormData, full_name: e.target.value })} />
              </div>
              <div className="form-control">
                ...
                <input type="email" placeholder="mario@email.com" className="input input-bordered w-full rounded-2xl bg-base-200/50" value={userFormData.email} onChange={e => setUserFormData({ ...userFormData, email: e.target.value })} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="form-control">
                  ...
                  <input type="text" placeholder="Mario" maxLength={10} className="input input-bordered w-full rounded-2xl bg-base-200/50" value={userFormData.alias} onChange={e => setUserFormData({ ...userFormData, alias: e.target.value })} />
                </div>
                <div className="form-control">
                  ...
                  <input type="tel" placeholder="+39 333..." className="input input-bordered w-full rounded-2xl bg-base-200/50" value={userFormData.phoneNumber} onChange={e => setUserFormData({ ...userFormData, phoneNumber: e.target.value })} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="form-control">
                  ...
                  <input type="text" placeholder="Es: Sviluppatore" className="input input-bordered w-full rounded-2xl bg-base-200/50" value={userFormData.work} onChange={e => setUserFormData({ ...userFormData, work: e.target.value })} />
                </div>
                <div className="form-control">
