...
export default function AdminPanel() {
  ...
  const fetchData = async () => {
    ...
    } catch (err) { console.error("Errore caricamento:", err); } finally { setLoading(false); }
  };
  useEffect(() => { fetchData(); }, []);
    ...
    <div className="animate-fade-in pb-20">
      <div className="flex justify-between items-end mb-6 border-b border-base-300 pb-4">
        ...
        <button onClick={() => handleOpenUserDialog()} className="group relative overflow-hidden rounded-xl bg-primary px-6 py-2 md:py-3 text-primary-content font-bold shadow-lg transition-all hover:scale-105 hover:shadow-primary/40">
          <div className="absolute inset-0 flex h-full w-full justify-center [transform:skew(-12deg)_translateX(-150%)] group-hover:duration-1000 group-hover:[transform:skew(-12deg)_translateX(150%)]"><div className="w-12 bg-white/30 blur-sm" /></div><span className="relative z-10">{t('admin.add_user')}</span>
        </button>
      </div>
          ...
          <tbody>
            {users.map((user, index) => (
              <tr key={user.id_user} className="stagger-item hover:bg-base-200/50 transition-colors duration-300" style={{ animationDelay: `${index * 50}ms` }}>
                ...
                <td className="text-right"><button onClick={() => handleOpenUserDialog(user)} className="btn btn-sm btn-ghost btn-circle text-info">✏️</button><button onClick={() => handleUserDelete(user.id_user, user.full_name)} className="btn btn-sm btn-ghost btn-circle text-error">🗑️</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="flex justify-between items-end mb-6 border-b border-base-300 pb-4 mt-8">
        ...
        <button onClick={() => handleOpenCategoryDialog()} className="group relative overflow-hidden rounded-xl bg-secondary px-6 py-2 md:py-3 text-secondary-content font-bold shadow-lg transition-all hover:scale-105 hover:shadow-secondary/40">
          <div className="absolute inset-0 flex h-full w-full justify-center [transform:skew(-12deg)_translateX(-150%)] group-hover:duration-1000 group-hover:[transform:skew(-12deg)_translateX(150%)]"><div className="w-12 bg-white/30 blur-sm" /></div><span className="relative z-10">{t('admin.add_category')}</span>
        </button>
      </div>
          ...
          <tbody>
            {categories.map((cat, index) => (
              <tr key={cat.id_category} className="stagger-item hover:bg-base-200/50 transition-colors duration-300" style={{ animationDelay: `${index * 50}ms` }}>
                <td className="font-mono text-base-content/50 text-xs">{cat.id_category}</td><td><div className="font-bold text-sm">{cat.name}</div></td><td className="text-center text-2xl">{cat.icon}</td><td className="text-right"><button onClick={() => handleOpenCategoryDialog(cat)} className="btn btn-sm btn-ghost btn-circle text-info">✏️</button><button onClick={() => handleCategoryDelete(cat.id_category, cat.name)} className="btn btn-sm btn-ghost btn-circle text-error">🗑️</button></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="modal-box bg-base-100 rounded-2xl p-6 md:p-8 max-w-md modal-smooth shadow-xl border border-base-200/50">
          ...
          <form onSubmit={handleUserSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="form-control"><label className="label pb-1"><span className="label-text text-base-content/70 font-medium">{t('admin.permissions')}</span></label>
                <select className="select select-bordered w-full bg-base-200 text-base-content border-base-300 focus:border-primary" value={userFormData.role} onChange={e => setUserFormData({ ...userFormData, role: e.target.value })}>
                  <option value="user">{t('admin.user_standard')}</option><option value="admin">{t('admin.user_admin')}</option>
                </select>
              </div>
            </div>
      ...
      <dialog className={`modal ${isCategoryModalOpen ? 'modal-open' : ''} modal-bottom sm:modal-middle z-50`}>
        <div className="modal-box bg-base-100 rounded-2xl p-6 md:p-8 max-w-md modal-smooth shadow-xl border border-base-200/50">
          ...
          <form onSubmit={handleCategorySubmit} className="space-y-4">
            <div className="form-control"><label className="label pb-1"><span className="label-text text-base-content/70">{t('admin.category_name')}</span></label><input type="text" required placeholder="Es: Ferie" className="input input-bordered w-full bg-base-200/30 focus:bg-base-200 focus:border-primary" value={categoryFormData.name} onChange={e => setCategoryFormData({ ...categoryFormData, name: e.target.value })} /></div>
            <div className="form-control col-span-1"><label className="label pb-1"><span className="label-text text-base-content/70 font-medium">{t('admin.icon')}</span></label>
              <select className="select select-bordered w-full text-xl bg-base-200 text-base-content border-base-300 focus:border-primary text-center" value={categoryFormData.icon} onChange={e => setCategoryFormData({ ...categoryFormData, icon: e.target.value })} required>
                <option value="" disabled>--</option><option value="🏢">🏢</option><option value="🏠">🏠</option><option value="🏖️">🏖️</option><option value="🤒">🤒</option><option value="💼">💼</option>
              </select>
            </div>
            <div className="modal-action pt-2 mt-6"><button type="button" onClick={handleCloseCategoryDialog} className="btn btn-ghost font-normal">{t('admin.cancel')}</button><button type="submit" className="btn btn-secondary px-8 font-medium">{categoryFormData.id_category ? t('admin.save') : t('admin.create')}</button></div>
          </form>
