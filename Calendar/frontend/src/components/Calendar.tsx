...
// 👇 1. AÑADIMOS LAS NUEVAS PROPS AQUÍ
        ...
        <table className="table table-fixed w-full border-separate border-spacing-0">
          ...
          <tbody className="divide-y-2 divide-base-300">
            {sortedUsers.map((user, index) => {
              ...
              // 👇 2. LÓGICA DE PERMISOS: ¿Es mi fila o soy admin?
                ...
                >
                  ...
                  {weekDays.map(day => {
                    ...
                    return (
                      <td 
                        key={dateStr} 
                        // 👇 3. AÑADIMOS EL ONCLICK AL TD DIRECTAMENTE PARA MAYOR COMODIDAD
                        onClick={() => {
                          if (canEdit) onAddPresence(user.id_user, dateStr);
                        }}
                        className={`p-0 border-r border-base-300 relative transition-all duration-300 
                          ${isWeekend ? 'bg-base-200/60' : ''} 
                          ${isToday ? 'bg-primary/5' : ''}
                          ${canEdit ? 'cursor-pointer hover:bg-base-200' : ''}`} // Feedback visual al pasar el ratón si puedes editar
                      >
                        {/* 👇 4. QUITAMOS EL pointer-events-none Y ARREGLAMOS LA FUNCIÓN */}
